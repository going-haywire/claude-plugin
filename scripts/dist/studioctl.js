...
async function portOwnerPid(port) {
 ...
 */
export async function resolveStudio(workspace, opts = {}) {
    ...
    const sidecarPort = identity?.pid !== undefined && pidAlive(identity.pid) ? identity.port : undefined;
 ...
 * of a new run.
 */
function announcedItself(now, before) {
    if (now.pid === undefined || !pidAlive(now.pid))
        return false;
    if (!before)
        return true;
 ...
 * full timeout.
 */
export async function startStudio(workspace, opts = {}) {
    ...
    if (resolved.state === "other" || resolved.state === "unknown") {
        throw new Error(`Port ${port} is already in use by ${resolved.state === "other"
            ? `another project (${resolved.other_identity?.project ?? "unknown"})`
            : "an unknown process"}. Resolve this before launching.`);
    }
    ...
    try {
        ...
        console.error(`studioctl: cannot write ${logPath} (${e.message}); discarding studio output.`);
